...
<head>
    ...
    <title>Nova Senha — Dinofy Master</title>
    ...
    <div class="bg-white shadow-sm rounded-2xl p-8 w-full max-w-md">
        ...
        <form method="POST" action="{{ route('admin.password.update') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="password" name="password" required autofocus class="w-full px-4 py-2.5 border border-zinc-300 rounded-lg text-sm focus:ring-2 focus:ring-green-600 focus:border-transparent transition">
            </div>
            <div>
                <label class="block text-sm font-medium text-zinc-700 mb-1">Confirmar Senha</label>
                <input type="password" name="password_confirmation" required class="w-full px-4 py-2.5 border border-zinc-300 rounded-lg text-sm focus:ring-2 focus:ring-green-600 focus:border-transparent transition">
            </div>
            <button type="submit" class="w-full py-2.5 px-4 text-white font-medium rounded-lg text-sm transition" style="background: var(--color-primary);" onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">Redefinir Senha</button>
        </form>
    </div>
</body>
</html>
